...
CCPhysicsNode*. Also create a code connection called *pullbackNode*:
...
Now we need add a joint between the invisible node and the catpult arm. This joint wil hold the catapult arm in its upright position. For this connection we will be using a *Physics Spring Joint*, because as you will see later, this will be a loosely connected and not a very stiff joint. Select it from the Node Library and drag it to the stage. Since the *pullbackNode* doesn't have a size, we need to drag the joint connection point to the timeline to connect to this node. We drag the second connection to the catapult arm:
...
Let's start implementing this feature in SpriteBuilder. **Create another hidden CCNode, just as you did in the last section for the pullbackNode** and create a code connection with the variable name *mouseJointNode* (*mouseJoint* is
a common name for a drag&drop joint*). The position of this *mouseJointNode* does not matter, since we will be moving it in code. When you finished all the steps, publish your project.
